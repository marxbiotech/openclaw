...
/** Minimal transport surface used to relay ACP events back to the gateway. */
export type AcpNodeEventTransport = {
  request(method: string, params?: unknown): Promise<unknown>;
};

async function sendNodeEvent(
  client: AcpNodeEventTransport,
  event: string,
  payload: unknown,
): Promise<void> {
  try {
...
async function handleSpawn(
  ...
  // Create acpx session for this cwd (acpx 0.1.16+ requires it). Failures must
  // be surfaced — a silent swallow here causes the subsequent acp.turn to fail
  // with a misleading "No acpx session found" error.
  try {
...
export function handleAcpEvent(evt: AcpEvent, client: AcpNodeEventTransport): void {
