...
import { resolvePackageRootFromGlobalRoot } from "../../infra/update-global.js";
...
export async function inspectNpmGlobalDestination(root: string, timeoutMs: number) {
  ...
  try {
    ...
    const packageRoot = resolvePackageRootFromGlobalRoot({
      globalRoot: destinationLayout.globalRoot,
      packageName: (await readPackageName(root)) ?? undefined,
      installedRoot: root,
    });
